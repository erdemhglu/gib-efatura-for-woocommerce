<?php
/**
 * Plugin Name:       WooCommerce GİB e-Fatura
 * Plugin URI:        https://github.com/erdemhglu/woo-gib-efatura
 ...
 * Author URI:        https://github.com/erdemhglu
 * Text Domain:       woo-gib-efatura
...
add_action( 'admin_notices', function () {
	if ( ! class_exists( 'WooCommerce' ) ) {
		echo '<div class="notice notice-error"><p>' .
			esc_html__( 'WooCommerce GİB e-Fatura eklentisinin çalışması için WooCommerce eklentisinin kurulu ve aktif olması gerekir.', 'woo-gib-efatura' ) .
			'</p></div>';
		return;
	}
	if ( ! wgf_autoload_ready() || ! class_exists( '\\Mlevent\\Fatura\\Gib' ) ) {
		echo '<div class="notice notice-error"><p>' .
			esc_html__( 'WooCommerce GİB e-Fatura: gerekli PHP kütüphaneleri bulunamadı. Eklenti klasöründe "composer install" komutunu çalıştırın (composer.json içinde mlevent/fatura paketi tanımlıdır).', 'woo-gib-efatura' ) .
			'</p></div>';
	}
} );
...
add_action( 'plugins_loaded', function () {
	load_plugin_textdomain( 'woo-gib-efatura', false, dirname( WGF_BASENAME ) . '/languages' );
